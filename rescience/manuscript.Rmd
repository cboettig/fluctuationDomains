...
Computational reproducibility is 
...
Note that in the modern standard for R packages with methods in implemented in C, several things would be different from my original implementation.  The C methods are expected to bind R's random number generator engines, and not use their own methods, which allows them to coordinate with standard R methods for setting seed and so forth.  Also, the data should be passed by reference back to R and not serialized to disk (though this can introduce memory constraints).  Third, it is now also standard practice to register the C routines explicitly.  The built-in CHECK methods of R will flag these issues.
...
While the package is organized as an R package, it is worth noting that it is also not written up to the standards required of modern R packages to pass the built-in `R CMD check` testing routines successfully.  Some of these are not particularly material 

- C routines are not registered
...
- The package does not use the R random number generator.  This was intentional, as the package was written as stand-alone C code that happened to have an R interface, though notably even the C implementation fails to expose a random number `seed`. 
- missmatch in documentation and function definition
